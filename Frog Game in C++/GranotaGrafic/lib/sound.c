...
int Sound_GetSoundQueueSlot(struct T_SOUND *sonido)
{
  ...
  // Si no tenemos un slot de reproducción libre nos marchamos sin hacer nada
...
//  memset(g_sound_buffer, 0, g_samples * NUM_CANALES_STEREO * sizeof(signed short)) ;
  ...
  // Inicializamos la cola de reproducción de sonidos
...
//  g_current_sound = sonido ;
  ...
  if(sonido->estado != SOUND_STATE_PLAYING) {
    ...
    // Si el sonido estaba parado pedimos una nueva posición en la cola
    ...
    // Si el sonido no estaba playeándose y no está en pause hay que reiniciarlo
    
    if(mode != SOUND_DO_NOT_RESTART_IF_ALREADY_PLAYING) Sound_Restart(sonido) ;
  }

  // Devolvemos la posición en la cola de reproducción
  return sonido->posicion_cola ;
}
...
{
  // Si el sonido no está reproduciéndose no hacemos nada

  if(sonido->posicion_cola == -1) return ;

  // Liberamos la posición de la cola que está ocupando el sonido
